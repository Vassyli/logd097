...
function pvpwarning($dokill=false) {
	...
	if ($session['user']['age'] <= $days &&
		...
		if ($dokill) {
			output("`\$Warnung!`^ Da du selbst noch vor PvP gesch�tzt warst, aber jetzt einen anderen Spieler angreifst, hast du deine Immunit�t verloren!!`n`n");
			$session['user']['pk'] = 1;
		} else {
			output("`\$Warnung!`^ Innerhalb der ersten $days  Tage in dieser Welt, oder bis sie $exp Erfahrungspunkte gesammelt haben, sind alle Spieler vor PvP-Angriffen gesch�tzt. Wenn du einen anderen Spieler angreifst, verf�llt diese Immunit�t f�r dich!`n`n");
		}
	}
}
...
function isnewday($level){
	...
	if ($session['user']['superuser']<$level) {
		...
		$session['bufflist']['angrygods']=array(
			"name"=>"`^Die G�tter sind w�tend!",
			"rounds"=>10,
			"wearoff"=>"`^Es ist den G�ttern langweilig geworden, dich zu qu�len.",
			"minioncount"=>$session['user']['level'],
			"maxgoodguydamage"=> 2,
			"effectmsg"=>"`7Die G�tter verfluchen dich und machen dir `^{damage}`7 Schaden!",
			"effectnodmgmsg"=>"`7Die G�tter haben beschlossen, dich erstmal nicht zu qu�len.",
			"activate"=>"roundstart",
			"survivenewday"=>1,
			"newdaymessage"=>"`6Die G�tter sind dir immer noch b�se!"
		);
		output("F�r den Versuch, die G�tter zu betr�gen, wurdest du niedergeschmettert!`n`n");
		output("`\$Ramius, der Gott der Toten`) erscheint dir in einer Vision. Daf�r, dass du versucht hast, deinen Geist mit seinem zu messen, sagt er dir wortlos, dass du keinen Gefallen mehr bei ihm hast.`n`n");
		addnews("`&F�r den Versuch, die G�tter zu besudeln, wurde ".$session['user']['name']." zu Tode gequ�lt! (Hackversuch gescheitert).");
		...
		while ($row = db_fetch_assoc($result)) {
			systemmail($row['acctid'],"`#{$session['user']['name']}`# hat versucht, Superuser-Seiten zu hacken!","B�se(r), b�se(r), b�se(r) {$session['user']['name']}, du bist ein Hacker!");
		}
		exit();
	}
}
...
function forest($noshowmessage=false) {
  ...
  if ($conf['healer'] || $session['user']['acctid']==getsetting("hasegg",0)) {
  	addnav("H?Golindas H�tte","healer.php");
  } else {
  	addnav("H?H�tte des Heilers","healer.php");
  }
  addnav("B?Etwas zum Bek�mpfen suchen","forest.php?op=search");
  ...
  addnav("","forest.php");
	if ($session['user']['level']>=15  && $session['user']['seendragon']==0){
		addnav("G?`@Den Gr�nen Drachen suchen","forest.php?op=dragon");
	}
	addnav("Sonstiges");
	...
	if ($noshowmessage!=true){
		...
		output("Der Wald, Heimat von b�sartigen Kreaturen und �blen �belt�tern aller Art.`n`n");
		output("Die dichten Bl�tter des Waldes erlauben an den meisten Stellen nur wenige Meter Sicht.  ");
		output("Die Wege w�rden dir verborgen bleiben, h�ttest du nicht ein so gut geschultes Auge. Du bewegst dich so leise wie ");
		output("eine milde Brise �ber den dicken Humus, der den Boden bedeckt. Dabei versuchst du es zu vermeiden ");
		output("auf d�nne Zweige oder irgendwelche der ausgebleichten Knochenst�cke zu treten, welche den Waldboden spicken. ");
		output("Du verbirgst deine Gegenwart vor den abscheulichen Monstern, die den Wald durchwandern.");
		if ($session['user']['turns']<=1) output(" In der N�he siehst du wieder den Rauch aus dem Kamin eines windschiefen Hexenh�uschens aufsteigen, von dem du schw�ren k�nntest, es war eben noch nicht da. ");
	}
...
function increment_specialty(){
  global $session;
		if ($session[user][specialty]>0){
			$skillnames = array(1=>"Dunkle K�nste","Mystische Kr�fte","Diebeskunst");
			...
			if ($x == 0){
				output("Du bekommst eine zus�tzliche Anwendung!`n");
				$session[user][$skillpoints[$session[user][specialty]]]++;
			}else{
				output("Nur noch ".(3-$x)." weitere Stufen, bis du eine zus�tzliche Anwendung erh�ltst!`n");
			}
		}else{
			output("`7Du wanderst ziel- und planlos durchs Leben. Du solltest eine Rast machen und einige wichtige Entscheidungen f�r dein weiteres Leben treffen.`n");
		}
}
...
function fightnav($allowspecial=true, $allowflee=true){
  global $PHP_SELF,$session;
	...
	addnav("K�mpfen","$script?op=fight");
	if ($allowflee) {
		addnav("Wegrennen","$script?op=run");
	}
	if (getsetting("autofight",0)){
		addnav("AutoFight");
		addnav("5 Runden k�mpfen","$script?op=fight&auto=five");
		addnav("Bis zum bitteren Ende","$script?op=fight&auto=full");
	}
	if ($allowspecial) {
		addnav("`bBesondere F�higkeiten`b");
...
// Angegebene Tags am Ende des Strings schlie�en
...
function charstats(){
	...
	if ($session[loggedin]){
		...
		while (list($key,$val)=each($session[bufflist])){
			$buffs.=appoencode("`#$val[name] `7($val[rounds] Runden �brig)`n",true);
		...
		if (getsetting("dispnextday",0)){
			...
			$charstat.=appoencode(templatereplace("statrow",array("title"=>"N�chster Tag","value"=>date("G\\h, i\\m, s\\s \\",strtotime("1980-01-01 00:00:00 + $realsecstotomorrow seconds")))),true);
		}
...
//   $paypalstr .= '</td></tr><tr><td span="2" align="right" valign="top"><a href="http://www.anpera.net" target="_blank"><img src="images/anpnet-klein.gif" alt="Sponsor" border="0"></a></td></tr></table>';
	...
	if ($session['user']['superuser']>1){
		...
		db_free_result($result);
		// Neue Petitionen; schauen, ob Sternchen n�tig ist
		$petitions['star'] = '';
		if ($petitions['unread']) {
...
function viewcommentary($section,$message="Kommentar hinzuf�gen?",$limit=10,$talkline="sagt") {
	global $HTTP_POST_VARS,$session,$REQUEST_URI,$HTTP_GET_VARS, $doublepost;
...
*/
	...
	for (;$i>=0;$i--){
		...
		if ($session[user][superuser]>=3 && $message=="X"){
			$out.="`0[ <a href='superuser.php?op=commentdelete&commentid=$commentids[$i]&return=".URLEncode($_SERVER['REQUEST_URI'])."'>L�schen</a> ]&nbsp;";
	...
	if ($session[user][loggedin]) {
		//if ($counttoday<($limit/2) || $session['user']['superuser']>=2){
			if ($message!="X"){
				...
				output("<form action=\"$REQUEST_URI\" method='POST'>`@$message`n<input name='insertcommentary[$section]' size='40' maxlength='".(200-$tll)."'><input type='hidden' name='talkline' value='$talkline'><input type='hidden' name='section' value='$section'><input type='submit' class='button' value='Hinzuf�gen'>`n".(round($limit/2,0)-$counttoday<3?"`)(Du hast noch ".(round($limit/2,0)-$counttoday)." Beitr�ge f�r heute �brig)":"")."`0`n</form>",true);
				addnav("",$REQUEST_URI);
			}
//		}else{
//			output("`@$message`nSorry, du hast deine Beitr�ge in dieser Region f�r heute aufgebraucht.`0`n");
//		}
	}
	...
	$req = preg_replace("'[&]?c(omscroll)?=([[:digit:]]|-)*'","",$REQUEST_URI)."&comscroll=0";
		...
		$req = preg_replace("'[&]?c(omscroll)?=([[:digit:]]|-)*'","",$REQUEST_URI)."&comscroll=".($com-1);
			...
			$req = str_replace("?&","?",$req);
			if (!strpos($req,"?")) $req = str_replace("&","?",$req);
			output(" <a href=\"$req\">N�chste &gt;&gt;</a>",true);
			addnav("",$req);
		}
	db_free_result($result);
}
...
if (file_exists("dbconnect.php")){
	...
	echo "Du must die ben�tigten Informationen in die Datei \"dbconnect.php.dist\" eintragen und sie unter dem Namen \"dbconnect.php\" speichern.".
	exit();
}
