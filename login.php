		...
		if(0){
			...
			if (db_num_rows($result)==1){
				...
				if ($session[user][emailvalidation]!="" && substr($session['user']['emailvalidation'],0,1)!="x"){
					$session[user]=array();
					$session[message]="`4Fehler: Du musst deine E-Mail Adresse best�tigen lassen, bevor du dich einloggen kannst.";
					echo $session[message];
					//header("Location: index.php");
					exit();
				}else{
				...
				$session[message]="`4Fehler: Login-Daten waren ung�ltig.`0";
